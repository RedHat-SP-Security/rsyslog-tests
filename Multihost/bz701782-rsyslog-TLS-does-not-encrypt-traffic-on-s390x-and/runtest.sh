...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#
#   runtest.sh of /CoreOS/rsyslog/Multihost/bz701782-rsyslog-TLS-does-not-encrypt-traffic-on-s390x-and
...
EOE
    ...
    logging_outputs:
      ...
      - name: forward_output
        ...
        tcp_port: 6514
        tls: true
        permitted_server: $syncSERVER_HOSTNAME
    logging_flows:
      - name: flows
        inputs: [system_input]
...
\$InputTCPServerStreamDriverPermittedPeer $syncCLIENT_HOSTNAME
...
module(
    ...
    PermittedPeer="$syncCLIENT_HOSTNAME"
    StreamDriver.Mode="1"
...
\$ActionSendStreamDriverPermittedPeer $syncSERVER_HOSTNAME
...
*.* action(type="omfwd"
    ...
    StreamDriverPermittedPeers="$syncSERVER_HOSTNAME")
EOF
        rlRun "rsyslogPrintEffectiveConfig -n"
    rlPhaseEnd
  fi
